...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setStringEncoding:NSISOLatin1StringEncoding];
    valueDict = [[[NSMutableDictionary alloc] init] autorelease];
...
- (void)setPostValue:(id <NSObject>)value forKey:(NSString *)key
{
    ...
    [valueDict setValue:[value description] forKey:key];
...
//	NSLog(@"self postData]2=%@",[[self postData]objectForKey:@"path"]);
	NSEnumerator *e = [[self postData] keyEnumerator];
	NSString *key;
	int i=0;
...
//       NSString *data = [NSString stringWithFormat:@"%@=%@%@", [self encodeURL:key], [self encodeURL:[[self postData] objectForKey:key]],(i<count ?  @"&" : @"")];
        NSString *data = [NSString stringWithFormat:@"%@=%@%@", key , [[self postData] objectForKey:key],(i<count ?  @"&" : @"")];
