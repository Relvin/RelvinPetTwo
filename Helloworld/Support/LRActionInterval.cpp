...
static inline float tuoyuanXat( float a, float c, float t )//返回X坐标}
{
	//参数方程}
...
//TuoyuanBy
//
LRTuoyuanBy* LRTuoyuanBy::create(float t, const lrTuoyuanConfig& c, float delta)//利用之前定义的椭圆的三个参数初始化椭圆}
{
	LRTuoyuanBy *pTuoyuanBy = new LRTuoyuanBy();
	pTuoyuanBy->initWithDuration(t, c, delta);
	pTuoyuanBy->autorelease();
    
	return pTuoyuanBy;
}

bool LRTuoyuanBy::initWithDuration(float t, const lrTuoyuanConfig& c, float delta)
{
...
//		float y = tuoyuanYat(a, by, c, time);
        ...
        float x = tuoyuanXat(a, c, time + m_fDelta);
        float y = tuoyuanYat(a, c, time + m_fDelta);
		m_pTarget->setPosition(ccpAdd(m_sConfig.centerPosition, ccp(x-a, y)));//由于我们画计算出的椭圆你做值是以原点为中心的，所以需要加上我们设定的中心点坐标}
	}
}
