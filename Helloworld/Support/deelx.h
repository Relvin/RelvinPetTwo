...
template <class CHART> CPosixElxT <CHART> :: CPosixElxT(const char * posix, int brightleft)
{
	...
	else if(!strncmp(posix, "blank:", 6)) m_posixfun =   isblank_ ;
	else                                  m_posixfun = 0         ;
}

inline int isblank_(int c)
{
	return c == 0x20 || c == '\t';
}
	...
	case RCHART('['):
		{
			...
			if( flags & IGNORECASE )
			{
				...
				for(i=0; i<oldcount; i++)
				{
					...
					{
						newmin = tolower( m_max(RCHART('A'), ranges[i*2  ]) );
						newmax = tolower( m_min(RCHART('Z'), ranges[i*2+1]) );
					...
					if( ranges[i*2] <= RCHART('z') && ranges[i*2+1] >= RCHART('a') )
					{
						newmin = toupper( m_max(RCHART('a'), ranges[i*2  ]) );
						newmax = toupper( m_min(RCHART('z'), ranges[i*2+1]) );
