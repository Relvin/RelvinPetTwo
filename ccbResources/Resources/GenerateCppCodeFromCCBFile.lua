...
local extensionMatchTypeTbl = extensionMatchTypeTbl;
--local ccbifilename = FILENAME or arg[1];
local arg = arg or {};
local filename = FILENAME or arg[1]
local classname = CLASSNAME or arg[2]
local outputfilename = OUTPUTFILENAME or arg[3] or classname 	--默认为类名
local outputpath = OUTPUTPATH or arg[4] or 'scenes/'	--默认当前文件夹的scenes/子文件夹
...
if file then
	...
	-- 检查customeName是否为空
	--local varCustomNameFlag = false;

	local varMenuFlag = false;
	local varControlFlag = false;
	local varCcbFlag = false;
	...
	while lineData do
		repeat
			...
			local ccbSelector = string.match(lineData, "(ccb/[^<]+)");
			if ccbSelector then
				varCcbFlag = true;
				table.insert(ccbFilesTbl, ccbSelector);
			end
...
--				showlog("find", "lineNum is ", lineCnt);
	...
	if varAssignmentFlag then
		...
		-- 	showlog(value)
		-- end);
		for key, value in pairs (varAssignmentTbl) do
			showlog(value)
		end
	end
--	table.foreach(varAssignmentTbl, function(key, value)
...
--	end);
	...
	if varMenuFlag then
		...
		-- 	showlog(value)
		-- end);
		for key, value in pairs (menuSelectorTbl) do
			showlog(value)
		end
	end
--	table.foreach(menuSelectorTbl, function(key, value)
...
--	end);
	...
	showlog("------control selector list:------");
	if varControlFlag then
		-- table.foreach(controlSelectorTbl, function(key, value)
		-- 	showlog(value);
		-- end);
		for key, value in pairs (controlSelectorTbl) do
			showlog(value)
		end
	end

	showlog("------ccb files list:------");
	if varCcbFlag then
		showlog("WARNING: add each ccb to your lib in scene() separete!!");
		for key, value in pairs (ccbFilesTbl) do
			showlog(value);
		end
	end
	...
	-- 引用的ccb文件
	local ccbFileBindTbl = {};
	local ccbFileImportTbl = {};

	-- 需release的绑定变量
	local needReleaseVariableTbl = {};
	...
	for idx, member in ipairs(varAssignmentTbl) do
		if duplicateTbl[member] ~= true then
			...
			local isNormalFlag = true;

			--if isExtensionFlag then
				for idx, types in ipairs(extensionMatchTypeTbl) do
	...
	local menuCallBackTpl = [[void %s::%s(CCObject* pSender)
{
	...
	-- 替换引用的ccb文件
	duplicateTbl = {}
	for idx, ccbVar in ipairs(ccbFilesTbl) do
		if duplicateTbl[ccbVar] ~= true then
			duplicateTbl[ccbVar] = true;
			table.insert(ccbFileBindTbl, string.format('pLoaderLib->registerCCNodeLoader( "[%s]", [%s]Loader::loader() );\n', ccbVar, ccbVar));
			table.insert(ccbFileImportTbl, string.format('#include "[%s]"', ccbVar));
		end
	end
...
--	showlog(ccbfilename);
	...
	local DataCache =
	{
		['$HeaderComeFrom'] = "//" .. outputfilename .. ".h come from " .. filename;
		['$ccbifilename'] = filename .. 'i';		-- ccbi文件名称
		['$classname'] = classname;					-- 当前类名称
		['$classLoader'] = classname .. 'Loader';
		...
		-- 替换引用ccb代码
		['$bindCcbFileSelector'] = table.concat(ccbFileBindTbl);
		['$bindCcbFileImportSelector'] = table.concat(ccbFileImportTbl);

		-- release绑定变量
		['$releaseAssigner'] = table.concat(needReleaseVariableTbl);
